...
public class Servlet_Upload extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {   
    	...
    	int vene_id = Integer.parseInt(session.getAttribute("vene_id").toString());
    	String kuva = "vene_" + vene_id + fileName;
    	System.out.println("kuvan nimi="+kuva);
        Dao_Vene dao = new Dao_Vene();
        try {
			dao.lisaaKuva(kuva, vene_id);
		} catch (Exception e) {			
			e.printStackTrace();
		}        
        request.setAttribute("message", "Kuva on ladattu onnistuneesti");
        getServletContext().getRequestDispatcher("/uploader.jsp?vene_id="+session.getAttribute("vene_id")).forward(request, response);
