...
<body>
    ...
    <ol id="sortable">
    <%
	session.setAttribute("vene_id", request.getParameter("vene_id"));	
	if(request.getAttribute("kuvat")!=null){
		String polku = "images/uploaded/";
		ArrayList<String> venekuvat=(ArrayList<String>)request.getAttribute("kuvat");	
		String kuva="";				
		for(int i=0; i<venekuvat.size();i++){
			kuva = venekuvat.get(i).replace("vene_" + request.getParameter("vene_id"),"");
...
<script>
	...
	$(document).ready(function(){	
		$("#sortable").sortable({
			 stop: function(event, ui) {
			        ...
			        $("#sortable li img").each(function(i, el){			           
			            ...
			            if(kuva!=null){
			            	kuva="vene_<%out.print(request.getParameter("vene_id"));%>" + kuva;			
			            	kuvaJarjestys+=kuva+";";
			            }			        	
			        });	
		...
		$(".poistakuva").click(function(){	
			var poistettava = "vene_<%out.print(request.getParameter("vene_id"));%>"+this.id;
			var r = confirm("Poista kuva " + this.id + "?");
			if (r == true) {
				$("#spinner").show();
				$.ajax({url: "Servlet_PoistaKuva?kuva_id="+poistettava, success: function(result){
					 document.location="Servlet_HaeKuvat?vene_id=<%out.print(request.getParameter("vene_id"));%>";
				}});
			}			 
		});		
	});
...
if(request.getAttribute("message")!=null){	
	response.sendRedirect("Servlet_HaeKuvat?vene_id=" + request.getParameter("vene_id"));
